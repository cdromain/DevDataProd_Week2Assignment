---
title: "Developing Data Product course - J.H. D.S.S. (Coursera)"
...
statedf <- data.frame(statedf, lat = c(32.806671, 61.370716, 33.729759, 34.969704,
                      ...
                      lng = c(-86.791130, -152.404419, -111.431221, -92.373123,
                              ...
                              -80.954453, -89.616508, -107.302490), col, stringsAsFactors = FALSE)
...
statedf %>% 
  ...
  addCircles(weight = 1, radius = sqrt(statedf$Population) * 1000, color = statedf$col) %>%
  addLegend(labels = c("less than 1%", "between 1 and 1.5%", "between 1.5 and 2%", "more than 2%"), colors = c("grey", "yellow", "orange", "red"), title = "Illiteracy")
```

```{r, eval=FALSE}
myFun <- function(x) {
        if (x$Illiteracy < 1) x$col == "white"
        if (x$Illiteracy >= 1 && x$Illiteracy < 1.5) x$col = "yellow"
        if (x$Illiteracy >= 1.5 && x$Illiteracy < 2) x$col = "orange"
        else x$col = "red"
}

statedf <- apply(statedf, 1, FUN = myFun)


statedf <- apply(statedf, 1, function(statedf) {
                    if (statedf$Illiteracy == "1.1") statedf$col == "red"
                 })
```
